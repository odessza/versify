...
- Used-songs exclusion: all previously used song+artist pairs are injected into the Gemini prompt to prevent song repetition
- Placeholder generation: a second Gemini call (parallel to Genius fetch) generates a short lyric-aware textarea placeholder, stored in `thought_placeholder` on the lyrics row
...
- **Schema**: `lyrics` table (id, date, lyric_text, song, artist, album_art_url, genius_url, thought_placeholder, published, approved, created_at) and `thoughts` table (id, lyric_id FK, user_id UUID, content, created_at). Index on thoughts(lyric_id, created_at desc).
- **Home**: fetches today's published lyric; submits Thoughts with anonymous user_id from localStorage. Redesigned with Fraunces display font for the lyric, Inter for all UI, dawn gradient background, rounded textarea with orange focus ring, pill Post Thought button, and `NowPlaying` vinyl component fixed bottom-right. Textarea placeholder uses `lyric.thought_placeholder` (generated by pipeline) with fallback to `"No one's watching. What's on your mind?"`.
...
- **`api/admin-auth.js`**, **`api/admin-lyrics.js`**, **`api/admin-approve.js`**, **`api/admin-regenerate.js`**, **`api/admin-generate.js`**, **`api/admin-publish.js`**: admin API endpoints, all protected by HMAC token (password never in client bundle). `admin-lyrics` returns `{ lyrics }` — all lyrics ordered by date desc, limit 90. `admin-generate` triggers tomorrow's draft on demand. `admin-publish` manually publishes today's approved lyric (same logic as the midnight cron, for use when cron misses). `admin-approve.js` is unused (approval is now automatic) but kept in place.
- **`api/_lyricPipeline.js`**: shared Gemini + Genius + Supabase insert logic used by generate and regenerate. Inserts with `approved=true`. Retries up to 3 times if duplicate lyric_text detected. Injects all previously used song+artist pairs into the Gemini prompt. Runs a second parallel Gemini call to generate `thought_placeholder`. Gemini calls retry up to 3 times on transient errors (429/503) with 30s → 60s → 90s backoff before failing.
...
- **Admin view** (`/admin`): password login → sessionStorage token. Two sections: (1) Action buttons — "Publish today" (orange, triggers `admin-publish` to manually publish today's approved lyric when the midnight cron misses) and "Generate tomorrow" (triggers `admin-generate` for cron recovery); (2) Lyrics table — all lyrics ordered by date desc, columns: Date / Lyric / Song & Artist / Status badge (Published · Scheduled · Draft) / Regenerate action (unpublished rows only). No manual approval step.
- **About page** (`/about`): personal "wtf is this about?" copy — 6 paragraphs, Dawn gradient background, Inter font, `max-w-[65ch]` centered layout, no extra chrome beyond the global Nav. A `wtf is this about?` link (muted, small, bottom-center) was added to the bottom of Home, Feed, and Archive.
